...
# 1. Mettre à jour les bots dans la BDD via Tinker
...
$nodeProcesses = Get-WmiObject Win32_Process | Where-Object { ($_.CommandLine -match "run_batch_processor.js" -or $_.CommandLine -match "simulation_bots.js") -and $_.Name -match "node" }
if ($nodeProcesses) {
...
Write-Host "Launching $BotCount simulation bots..." -ForegroundColor Cyan
Start-Process -NoNewWindow -FilePath "node" -ArgumentList "smart_contracts/simulation_bots.js $BotCount 2" -WorkingDirectory "$PSScriptRoot\.." -RedirectStandardOutput "$LogDir\simulation.log" -RedirectStandardError "$LogDir\simulation_err.log" -PassThru

Write-Host "`n✅ Done! The batch processor and simulation bots are now running with BASE_BET=$BaseBet." -ForegroundColor Green
