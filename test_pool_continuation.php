...
// Let's set initial = 0.5. Bet = 0.1.
// Winning makes it 0.6. q = 0.6 / 0.5 = 1.2 (which is < 2.0).
...
$user1->balance = 0.4; // 0.4 + (0.1 coeff) = 0.5 initial
...
$request1 = Request::create('/api/store-pre-moves', 'POST', ['user_id' => $user1->id, 'bet_amount' => 0.1, 'cid' => $cid1, 'pre_moves' => ['rock', 'rock']]);
$controller->storePreMoves($request1);

$request2 = Request::create('/api/store-pre-moves', 'POST', ['user_id' => $user2->id, 'bet_amount' => 0.1, 'cid' => $cid2, 'pre_moves' => ['scissors', 'scissors']]);
...
// Manually trigger SessionFinishedEvent for the winner, as `PoolLifecycleService->finishPool` 
// is normally responsible for this but might not be reached in a single match direct execution if pool loops.
// Actually, `processPoolAutoMatch` -> while(users >=2) execute, then `finishPool(pool)`.
// Since we have 2 users, 1 gets eliminated/doubled. Loop ends. `finishPool` should be called.
// BUT in `finishPool`: $user->balance += $user->battle_balance; $user->battle_balance = 0; THEN event(new SessionFinishedEvent).
// Let's check if the event was actually fired or if we need to fire it manually for the test.
logOutput("Triggering SessionFinishedEvent for User 1 manually via Listener...");
$listener = app(\App\Listeners\SessionFinishedEventListener::class);
$event = new \App\Events\SessionFinishedEvent($user1->id, $dbPool);
...
if ($user2->balance < 0.4) {
    // If they have enough to play again (balance > 0.2 because bet doubled)
